...
public class Ex04_InnerClass {
	...
	//		특징) 내부 클래스에서는 외부클래스의 멤버들을 마음대로 사용가능 ( 상속 비슷 ) 
	//			상속에서는 private 접근 안됨 , but 내부클래스에서는 private 접근이 가능
	//			외부 클래스를 통해서만 내부클래스를 생성할 수 있다.(내부 클래스만 별도로 생성 불가능)
	
	//	2. Local : 외부클래스 메서드 안에 내부 클래스가 존재
	//				외부 클래스가 지역변수처럼 사용
	//				외부 클래스의 메서드가 호출(실행) 될때 생성 할 수 있다.
	...
	//	3. static 내부클래스 : 구조는 Member 내부 클래스와 같다.
	//					 	내부 클래스의 멤버 변수는 static을 사용할 수 없다.
	//						어쩔 수 없이 내부클래스의 맴버변수를 static으로 사용할 때 

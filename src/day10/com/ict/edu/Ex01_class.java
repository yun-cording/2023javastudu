...
public class Ex01_class {
	...
	//						 기능 = 동작 = 멤버 메서드
	//						 생성자 = 클래스가 객체로 맏들어질때 한번
	//						 호출 되서 데이터를 초기화 할 수 있는 기능을 가졌다.
	// 						 ** 클래스 이름 = 저장이름 = 파일이름 = 생성자이름
	
	// 클래스 구조 : 헤더 , 멤버필드 , 생성자 , 멤버메서드
	//	1. 헤더 : [접근 제한자](생략가능하다)  [클래스 종류에 대한 예약어] class 클래스이름{    }
	// ex)public class Ex01_class {  public = 접근 제한자
	// 	1) 접근제한자 : 클래스 , 멤버필드 , 멤버메서드 , 생성자 모두 사용할 수 있다.
	//		외부에서 접근할 수 있는 권한의 단계를 말함.
	//		가) public : 누구나 다 접근 가능 == 사용가능
	...
	//				첫글자 대문자 나머지는 소문자.
	//				2단어 이상일때는 단어의 첫글자들은 대문자
	//				숫자는 중간이나 뒤에 사용 , 공백은 사용 불가
	
	//	2. 멤버필드 : 데이터 , 상태값 , 속성
	//		1) 변수 : 언제든지 변할 수 있는 데이터를 저장하는 공간
	//				첫글자 소문자 2단어 이상일때는 단어의 첫글자들은 대문자 
	//				숫자는 중간이나 뒤에 사용 , 공백은 사용 불가
	...
	//	public 누구다 접근 / private 누구나 다 접근못함 / 생략(default) 같은 패키지에서만 접근 / protected 상속관계 또는 같은 패키지만
	...
	//				숫자는 중간이다 뒤에 사용 , 공백은 사용 불가
	//	3) ★☆인자 = 매개변수 : 보통은 실행할때 필요한 정보를 외부에서 받기 위한 통로
	//				생략 가능하고 여러개 사용 가능
	//	4) ★☆오버로딩(중복정의) : 메서드 이름이 같으나 인자 수나 인자 자료형이 다르면 다른 메소드로 판별한다.
	...
	//				 반환형이 없는 메서드의 형태이다.
	//				 [접근제한자] 생성자(클래스이름)([인자]){ 실행 내용 } 
	//				 인자가 없는 생성자를 기본 생성자라고 한다.
	//		생성자 오버로딩 가능,
	//		하나의 클래스의 여러개의 생성자가 존재할 수 있다. => 생성자 오버로딩
	//		★☆모든 클래스는 반드시 생성자를 가지고 이씀
	//		해당 클래스의 생성자가 없으면 기본 생성자가 생략되어 있음 
	//		해당 클래스의 생성자가 없으면 객체를 만들때 기본 생성자를 호출해서 객체를 만든다.
	...
	//		ex) 참조변수.멤버필드,	참조변수.멤버메서드() , (단, 접근제한자를 확인)
